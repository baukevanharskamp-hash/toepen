  ...
  return (
    <main className="mx-auto flex min-h-dvh max-w-md flex-col px-5 pb-8 pt-[max(1.5rem,env(safe-area-inset-top))]">
      ...
      <section className="flex flex-1 flex-col justify-center py-10 text-center">
        <div className={`mx-auto grid h-28 w-28 place-items-center rounded-full border ${isFinale ? "border-lime/30 bg-lime/10" : "border-[#e86c5d]/30 bg-[#e86c5d]/10"}`}>
          <PlayerAvatar avatar={(isFinale ? winner?.avatar : loser?.avatar) ?? "🃏"} size="lg" />
        </div>
        ...
        <div className="mt-6 grid grid-cols-2 gap-2">
          {game.players.map((player) => (
            <div key={player.id} className="flex items-center gap-2 rounded-2xl border border-white/10 bg-white/[.04] p-3 text-left">
              <PlayerAvatar avatar={player.avatar} size="xs" /><span className="min-w-0 flex-1 truncate text-xs font-black">{player.name}</span><span className="text-xl font-black">{player.score}</span>
            </div>
          ))}
        </div>
      </section>
