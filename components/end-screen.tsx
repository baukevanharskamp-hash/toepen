...
export function EndScreen({ game, goHome }: { game: PublicGame; goHome: () => void }) {
  ...
  async function share() {
    const text = `${loser?.name} verloor ons potje Toepen met ${loser?.score} punten. Opdracht: ${game.task}`;
  ...
  return (
    <main className="mx-auto flex min-h-dvh max-w-md flex-col px-5 pb-8 pt-[max(1.5rem,env(safe-area-inset-top))]">
      ...
      <section className="flex flex-1 flex-col justify-center py-10 text-center">
        <div className="mx-auto grid h-24 w-24 place-items-center rounded-full border border-[#e86c5d]/30 bg-[#e86c5d]/10 text-5xl">{loser?.avatar}</div>
        <div className="mt-6 text-xs font-black uppercase tracking-[.24em] text-[#ff8d80]">We hebben een verliezer</div>
        <h1 className="mt-2 text-5xl font-black leading-none tracking-tight">{loser?.name}<br/><span className="text-cream/30">is af.</span></h1>
        <div className="mt-8 rounded-[28px] border border-amber/25 bg-amber/[.08] p-6">
          <div className="text-[10px] font-black uppercase tracking-[.2em] text-amber">Opdracht uitvoeren</div>
